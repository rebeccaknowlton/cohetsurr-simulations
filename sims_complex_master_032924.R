...
delta.table.two.step <- matrix(ncol = 16, nrow = 6)
 ...
 delta.table.two.step[1,] <- colMeans(list.output.2$delta - list.output.2$delta.bias, na.rm=TRUE)
  ...
  latex.table(delta.table.two.step, paste0("simrestwo_", sim.num, "_delta"), caption = "", dcolumn = T, dec = 3)
  latex.table(delta.s.table.two.step, paste0("simrestwo_", sim.num, "_delta.s"), caption = "", dcolumn = T, dec = 3)
  latex.table(R.s.table.two.step, paste0("simrestwo_", sim.num, "_R.s"), caption = "", dcolumn = T, dec = 3)
...
######
print(round(cbind(delta.table,delta.table.two.step),4))
print(round(cbind(delta.s.table,delta.s.table.two.step),4))
print(round(cbind(R.s.table,R.s.table.two.step),4))
...
get.truth = function(setting, grid){
  ...
  if(setting ==3){
    ...
    #beta.1 and #beta.0 are zero
    delta.s = (params$beta2+params$beta3)*params$s.mean.control+ 2*(grid%*%params$beta5)  - (params$beta2)*(params$s.mean.control) - grid%*%params$beta5
    delta = (params$beta2+params$beta3)*(grid%*%params$beta5 + params$s.mean.treat) + 2*(grid%*%params$beta5) - (params$beta2)*(params$s.mean.control) - grid%*%params$beta5
    R = 1-delta.s/delta
    return(list("delta.s" = delta.s, "delta" = delta, "R" = R))
  }
...
gen.data = function(n,setting){
	...
	if(setting == 1) {
   	...
   	data.temp$S =vector(length=n*2)
  		...
  		data.temp$S[data.temp$A==0]= rnorm(sum(data.temp$A==0), params$s.mean.control, params$s.sd.control)
...
if(setting == 2) {
    ...
    if(setting == 3) {
		data.temp <- data.frame(A = c(rep(1,n),rep(0,n)),
  		...
  		data.temp$S =vector(length=n*2)
  		data.temp$S[data.temp$A==1]= as.matrix(cbind(data.temp$W1[data.temp$A==1],data.temp$W2[data.temp$A==1]))%*%params$beta5 + rnorm(sum(data.temp$A==1), params$s.mean.treat , params$s.sd.treat )
  		data.temp$S[data.temp$A==0]= rnorm(sum(data.temp$A==0), params$s.mean.control, params$s.sd.control)
...
# Setting 2 = models wrong + yes hetero
# Setting 3 = model wrong + no hetero
...
for (i in 1:num.sim) {
  ...
  func.output <- complex.heterogeneity(y = data.temp$Y,
                                       ...
                                       variance = TRUE,
                                       test = TRUE,
                                       W.grid = w1w2.grid, threshold=threshold)
  outputfile <- rbind(outputfile,func.output$return.grid)
  pfile = rbind(pfile, c(func.output$pval, func.output$band.critical))
print(pfile)

 write.table(outputfile, paste("outputfile", setting, "_032924",parallel.num,".txt", sep=""), quote = FALSE, row.names = FALSE)
 write.table(pfile, paste("pfile", setting, "_032924_",parallel.num,".txt", sep=""), quote = FALSE, row.names = FALSE)
