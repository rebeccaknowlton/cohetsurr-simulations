...
for(i in 1:dim(pfile)[1]) {
	...
	setting.two.step$pval.threshold[i,] <- results.temp$pval.threshold.two.step
 	...
 	band.hold.lower.p = rbind(band.hold.lower.p, results.temp$R.s - pfile[i,4]*sqrt(results.temp$R.s.var))
 band.hold.upper.p = rbind(band.hold.upper.p, results.temp$R.s + pfile[i,4]*sqrt(results.temp$R.s.var))
 band.hold.lower.t = rbind(band.hold.lower.t, results.temp$R.s.two.step - pfile[i,5]*sqrt(results.temp$R.s.var.two.step))
 band.hold.upper.t = rbind(band.hold.upper.t, results.temp$R.s.two.step + pfile[i,5]*sqrt(results.temp$R.s.var.two.step))
	}
...
prop.p = colMeans(pval.adj.p < 0.05)
...
prop.t = colMeans(pval.adj.ts < 0.05)

if(kk == 1) {
	setting1.region = cbind(W.grid.expand, truth$R,prop.p, prop.t, apply(band.hold.lower.p, 2, function(x) mean(x>threshold.1)), apply(band.hold.lower.t, 2, function(x) mean(x>threshold.1)))
}
if(kk == 2) {
	setting2.region = cbind(W.grid.expand, truth$R,prop.p, prop.t, apply(band.hold.lower.p, 2, function(x) mean(x>threshold.2)), apply(band.hold.lower.t, 2, function(x) mean(x>threshold.2)))
}
if(kk == 3) {
	setting3.region = cbind(W.grid.expand, truth$R,prop.p, prop.t,apply(band.hold.lower.p, 2, function(x) mean(x>threshold.3)), apply(band.hold.lower.t, 2, function(x) mean(x>threshold.3)))
}
}	
...
names(setting1.region) = c("W1","W2", "truth", "parametric","twostep", "parametricband","twostepband")
setting2.region = as.data.frame(setting2.region)
names(setting2.region) = c("W1","W2", "truth", "parametric","twostep","parametricband","twostepband")
setting3.region = as.data.frame(setting3.region)
names(setting3.region) = c("W1","W2", "truth", "parametric","twostep","parametricband","twostepband")
...
p.1.p = ggplot(setting1.region, aes(W1, W2, fill= parametricband)) + 
  geom_tile() + 
  scale_fill_gradient(low="grey", high="red", limits = c(min.z,max.z)) + 
  xlab("W1") +
  ylab("W2") +
...
p.1.t = ggplot(setting1.region, aes(W1, W2, fill= twostepband)) + 
  geom_tile() + 
  scale_fill_gradient(low="grey", high="red", limits = c(min.z,max.z)) + 
  xlab("W1") +
  ylab("W2") +
...
p.2.p = ggplot(setting2.region, aes(W1, W2, fill= parametricband)) + 
  geom_tile() + 
  scale_fill_gradient(low="grey", high="red", limits = c(min.z,max.z)) + 
  xlab("W1") +
  ylab("W2") +
...
p.2.t = ggplot(setting2.region, aes(W1, W2, fill= twostepband)) + 
  geom_tile() + 
  scale_fill_gradient(low="grey", high="red", limits = c(min.z,max.z)) + 
  xlab("W1") +
  ylab("W2") +
...
p.3.p = ggplot(setting3.region, aes(W1, W2, fill= parametricband)) + 
  geom_tile() + 
  scale_fill_gradient(low="grey", high="red", limits = c(min.z,max.z)) + 
  xlab("W1") +
  ylab("W2") +
...
p.3.t = ggplot(setting3.region, aes(W1, W2, fill= twostepband)) + 
  geom_tile() + 
  scale_fill_gradient(low="grey", high="red", limits = c(min.z,max.z)) + 
  xlab("W1") +
  ylab("W2") +
